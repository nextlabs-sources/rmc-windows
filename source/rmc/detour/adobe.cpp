...
static ADOBE_MAPISENDMAIL				g_fnorg_MAPISendMail = NULL;
static ADOBE_MAPISENDMAIL				g_fn_MAPISendMail_trampoline = NULL;
static ADOBE_MAPISENDMAIL				g_fnorg_MAPISendMailW = NULL;
static ADOBE_MAPISENDMAIL				g_fn_MAPISendMailW_trampoline = NULL;

static ADOBE_XI_SENDMAIL				g_fnorg_AdobeXISendMail = NULL;
static ADOBE_XI_SENDMAIL				g_fn_AdobeXISendMail_trampoline = NULL;

static CREATEPROCESSW					g_fn_org_CreateProcessW = NULL;
static CREATEPROCESSW					g_fn_CreateProcessW_trampoline = NULL;
...
void __cdecl Core_OnAVAppFrontDocDidChange(AVDoc doc, void *clientData)
{
	...
	do 
	{
		...
		// take care of rights
		//
		hr = NX::NXL::RAW::CheckRights(FileName, &RightsMask, &CustomRightsMask, &EvaluationId);
		...
		if (pPageView)
		{
			...
			if (WindowInfo)
			{
				...
				FOR_EACH_LIST(ite, &Global.PageViewCtxList)
				{
					...
					if (PageViewCtx->hWnd == WindowInfo->hWnd && PageViewCtx->Doc == doc)
					{
						break;
					}
					else
...
void __cdecl Core_OnAVDocWillClose(AVDoc doc, void *clientData)
{
	...
	if (Ctx && Ctx->Doc == doc)
	{
		...
		{
			hCompositionWnd = Ctx->hCompositionWnd;
			...
			FOR_EACH_LIST(ite, &Global.PageViewCtxList)
			{
				...
				if (pNode->hCompositionWnd == Ctx->hCompositionWnd && pNode != Ctx)
				{
					hCompositionWnd = NULL;
					break;
				}
			}
			...
			if (hCompositionWnd)
				g_fnorg_SetWindowDisplayAffinity(hCompositionWnd, WDA_NONE);
		}

		if (Ctx->hOverlayWnd)
		{
			hOverlayWnd = Ctx->hOverlayWnd;
			...
			FOR_EACH_LIST(ite, &Global.PageViewCtxList)
			{
				...
				if (pNode->hOverlayWnd == Ctx->hOverlayWnd && pNode != Ctx)
				{
					hOverlayWnd = NULL;
					break;
				}
			...
			if (hOverlayWnd)
				send_overlay_windows_info_to_engine(hOverlayWnd, FALSE);
		}
...
HRESULT WINAPI Core_MAPISendMail(_In_ LPVOID   lhSession, _In_ ULONG_PTR     ulUIParam, _In_ LPVOID lpMessage, _In_ unsigned long    flFlags, ULONG         ulReserved)
{
	HRESULT hr = S_OK;
	...
	do
	{
		...
		return 2;// MAPI_E_FAILURE

	} while (FALSE);

	return g_fn_MAPISendMail_trampoline(lhSession, ulUIParam, lpMessage, flFlags, ulReserved);
}

HRESULT WINAPI Core_MAPISendMailW(_In_ LPVOID   lhSession, _In_ ULONG_PTR     ulUIParam, _In_ LPVOID lpMessage, _In_ unsigned long    flFlags, ULONG         ulReserved)

{
	...
	return g_fn_MAPISendMailW_trampoline(lhSession, ulUIParam, lpMessage, flFlags, ulReserved);
}
...
static BOOL InitializeAdobeSendMailHooksX(void)
{
	...
	do 
	{
		...
		do 
		{
			...
			g_fnorg_MAPISendMail = (ADOBE_MAPISENDMAIL)GetProcAddress(Global.Mapi32Handle, ADOBE_MAPISENDMAIL_PROC_NAME);
			if (g_fnorg_MAPISendMail) {
				//ignore error
				install_hook(g_fnorg_MAPISendMail, (PVOID*)&g_fn_MAPISendMail_trampoline, Core_MAPISendMail);
			}

			g_fnorg_MAPISendMailW = (ADOBE_MAPISENDMAIL)GetProcAddress(Global.Mapi32Handle, ADOBE_MAPISENDMAILW_PROC_NAME);
			if (g_fnorg_MAPISendMailW) {
				//ignore error
				install_hook(g_fnorg_MAPISendMailW, (PVOID*)&g_fn_MAPISendMailW_trampoline, Core_MAPISendMailW);
			}
...
BOOL InitializeAdobeCreateProcessHook(void)
{
	HMODULE hKernel;
	if (!Global.IsWin7) {
		hKernel = GetModuleHandleW(L"kernelbase.dll");
	}
	else {//For Windows 7, hook CreateProcessW at kernel32.dll
		hKernel = GetModuleHandleW(L"kernel32.dll");
	}
	if (hKernel)
	{
		g_fn_org_CreateProcessW = (CREATEPROCESSW)GetProcAddress(hKernel, "CreateProcessW");

		if (g_fn_org_CreateProcessW)
		{
			install_hook(g_fn_org_CreateProcessW, (PVOID*)&g_fn_CreateProcessW_trampoline, Core_CreateProcessW);
			Global.AcroCreateProcessInitialized = TRUE;
		}
	}

	return Global.AcroCreateProcessInitialized;

}

void CleanupAdobeCreateProcessHook(void)
{
	if (g_fn_CreateProcessW_trampoline) {
		remove_hook(g_fn_CreateProcessW_trampoline);
		g_fn_CreateProcessW_trampoline = NULL;
	}
}
...
static BOOL WINAPI Core_CreateProcessW(
...
)
{
	...
	do {
		...
		if (lpApplicationName == NULL) {//not possible for 32bit or 64 bit application
			break;
		}

		if (wcsstr(lpApplicationName, ADOBEEMAIL_MODULE_NAME_32) || wcsstr(lpApplicationName, ADOBEEMAIL_MODULE_NAME_64)) {//block adobe launch 32bitmapibroker.exe or 64bitmapibroker.exe
			SetLastError(ERROR_FILE_NOT_FOUND);
			send_block_notification(L"", NxrmdrvAdobeSendEmailBlocked);
			return FALSE;
		}
	} while (FALSE);

	return g_fn_CreateProcessW_trampoline(lpApplicationName,
		lpCommandLine,
		lpProcessAttributes,
		lpThreadAttributes,
		bInheritHandles,
