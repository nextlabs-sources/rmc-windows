			...
			if (SUCCEEDED(hrlocal))
			{
				do 
				{
					...
					if (Global.IsMsOffice || Global.IsAdobeReader)
					{
						...
						// take care of rights
						//
						hrlocal = NX::NXL::RAW::CheckRights(ActiveDocName, &RightsMask, &CustomRightsMask, &EvluationId);
...
static HRESULT takecare_winword_insert_file(WCHAR *FileName)
{
	...
	do 
	{
		...
		// take care of rights
		//
		hrlocal = NX::NXL::RAW::CheckRights(FileName, &RightsMask, &CustomRightsMask, &EvluationId);
