...
static GETCLASSFILEEX	g_fnorg_GetClassFileEx = NULL;
static GETCLASSFILEEX	g_fn_GetClassFileEx_trampoline = NULL;
	...
	/* [out][in] */ __RPC__inout DWORD *pdwEffect)
{
	...
	do 
	{
		...
		{
			if (Global.IsWin10 && Global.IsBrower)
			{
				hr = NX::NXL::RAW::IsDecryptedFile2(FileName);
			}
			else
			{
				hr = NX::NXL::RAW::IsDecryptedFile(FileName);
			}

			if (hr == S_OK && (!is_file_name_end_with_nxl(FileName)))
			{
				bFoundNXLFile = TRUE;
...
BOOL InitializeOLE2CtlHook(void)
{
	...
	do 
	{
		...
		if (g_fnorg_GetClassFile)
		{
			...
			{
				g_fnorg_GetClassFileEx = (GETCLASSFILEEX)((ULONG_PTR)g_fnorg_GetClassFile + 0x14);

				if (g_fnorg_GetClassFileEx)
				{
					if (!install_hook(g_fnorg_GetClassFileEx, (PVOID*)&g_fn_GetClassFileEx_trampoline, Core_GetClassFileEx))
					{
						bRet = FALSE;
						break;
					}
...
void CleanupOLE2CtlHook(void)
{
	...
	if (g_fn_GetClassFileEx_trampoline)
	{
		remove_hook(g_fn_GetClassFileEx_trampoline);
		g_fn_GetClassFileEx_trampoline = NULL;
	}
}
	...
	_Out_ CLSID FAR* pclsid)
{
	...
	do 
	{
		...
		// take care of rights
		//
		hr = NX::NXL::RAW::CheckRights(szFilename, &RightsMask, &CustomRightsMask, &EvaluationId);
...
HRESULT WINAPI Core_GetClassFileEx(
	_In_ LPCOLESTR szFilename,
	_Out_ CLSID FAR* pclsid,
	_In_ CLSID *guid)
{
	...
	do
	{
		...
		if (memcmp(guid, &GUID_NULL, sizeof(GUID_NULL)) != 0)
		{
			break;
		}
	...
	if (bAllow)
	{
		return g_fn_GetClassFileEx_trampoline(szFilename, pclsid, guid);
	}
	else
	{
		send_block_notification(szFilename, NxrmdrvEmbeddedOleObjBlocked);
...
static BOOL is_file_name_end_with_nxl(const WCHAR *FileName)
{
	...
	do
	{
		...
		if (FileNameLength < sizeof(DRAGDROP_NXL_EXT) / sizeof(WCHAR))
		{
			break;
		}
		...
		p -= (sizeof(DRAGDROP_NXL_EXT) / sizeof(WCHAR) - 1);

		bRet = _wcsicmp(p, DRAGDROP_NXL_EXT) == 0 ? TRUE : FALSE;
...
BOOL InitializeGoogleDrvSyncHook(void)
{
	BOOL bRet = TRUE;
	...
	do
	{
		if (Global.GoogleDrvSyncHooksInitialized)
		{
			break;
		}
		...
		Global.GoogleDrvSyncHooksInitialized = TRUE;
...
void CleanupGoogleDrvSyncHook(void)
{
	if (g_fn_FindFirstFileW_trampoline)
	{
		remove_hook(g_fn_FindFirstFileW_trampoline);
...
)
{
	...
	do 
	{
		...
		//
		hr = NX::NXL::RAW::CheckRights(lpFileName, &RightsMask, &CustomRightsMask, &EvaluationId);
...
BOOL IsValidNXLFile(const WCHAR *FileName)
{
	...
	do 
	{
		...
		if (signature.magic.code != NXL_MAGIC_CODE_2) 
		{
			break;
		}

		
		if (HIWORD(signature.version) >= 2 && file_size.QuadPart > sizeof(NXL_HEADER_2))
		{
			bRet = TRUE;
		}
