...
//return bytes_to_save. 0 if failed. 
static DWORD build_trampoline_call_from_original_x64(PVOID original_call, PVOID trampoline_call,DWORD bytes_to_save,PVOID staging_jmp);
...
__declspec(dllexport) BOOL install_hook_x64(
	PVOID original_call, 
	PVOID *trampoline_call, 
	PVOID new_call)
{
	...
	do 
	{
		...
		bytes_to_save = build_trampoline_call_from_original_x64(original_call, pfntrampoline, bytes_to_save, staging_jmp);
		if(!bytes_to_save)
		{
			bRet = FALSE;
			break;
...
static DWORD build_trampoline_call_from_original_x64(
	PVOID original_call, 
	PVOID trampoline_call,
	DWORD bytes_to_save,
	PVOID staging_jmp)
{
	DWORD dwRet = bytes_to_save;
	...
	do 
	{
		...
		__try
		{
			...
			{
				bytes_to_save = 5;//only copy 5 bytes of original function.

				//
				// setup trampoline jmp
				//
				*(ULONG_PTR*)(trampoline_jmp + 3) = (ULONG_PTR)((UCHAR*)staging_jmp + 14);

				//
				// copy trampoline jmp
				//
				memcpy((BYTE*)(trampoline_call),
					trampoline_jmp,
					sizeof(trampoline_jmp));
...
static PVOID build_staging_jmp_x64(PVOID original_call, PVOID new_call)
{
	...
	BYTE staging_jmp[32] = { 0x48,0xb8,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,\
		0xff, 0xe0, 0x90, 0x90, \
		0x58, 
	...
	do 
	{
		...
		if(p)
		{
			...
			*(ULONG_PTR*)(staging_jmp + 2) = (ULONG_PTR)new_call;
			//following instruction only takes affect when original call instruction is 0xe9.
			*(DWORD*)(staging_jmp + 16) = (DWORD)((((BYTE*)original_call + 5) + *(DWORD*)((BYTE*)original_call + 1)) - ((BYTE*)p + 20)); 
