...
time::formatted_span::formatted_span() : _days(0), _hours(0), _minutes(0), _seconds(0)
{
}

time::formatted_span::formatted_span(__int64 seconds)
{
	if (seconds > (__int64)SecondsPerDay) {
		_days = (int)SecondsToDays(seconds);
		seconds = (seconds % SecondsPerDay);
	}
	if (seconds > (__int64)SecondsPerHour) {
		_hours = (int)SecondsToHours(seconds);
		seconds = (seconds % SecondsPerHour);
	}
	if (seconds > (__int64)SecondsPerMinute) {
		_minutes = (int)SecondsToMinutes(seconds);
		seconds = (seconds % SecondsPerMinute);
	}
	_seconds = (int)seconds;
}

time::formatted_span::formatted_span(const formatted_span& rhs)
	: _days(rhs._days), _hours(rhs._hours), _minutes(rhs._minutes), _seconds(rhs._seconds)
{
}

time::formatted_span::~formatted_span()
{
}

time::formatted_span& time::formatted_span::operator = (const time::formatted_span& rhs)
{
	if (this != &rhs) {
		_days = rhs._days;
		_hours = rhs._hours;
...
std::wstring time::formatted_span::serialize()
{
	std::wstring s;
	if (_days > 0) {
		s.append(NX::itos<wchar_t>(_days));
		s.append((_days > 1) ? L" Days " : L" Day ");
	}
	if (_hours > 0) {
		s.append(NX::itos<wchar_t>(_hours));
		s.append((_hours > 1) ? L" Hours " : L" Hour ");
	}
	if (_minutes > 0) {
		s.append(NX::itos<wchar_t>(_minutes));
		s.append((_minutes > 1) ? L" Minutes " : L" Minute ");
	}
	if (_seconds > 0) {
		s.append(NX::itos<wchar_t>(_seconds));
		s.append((_seconds > 1) ? L" Seconds" : L" Second");
	}
	return s;
}

std::wstring time::formatted_span::serialize_timepassed()
{
	std::wstring s;

	if (_days >= 365) {
		const int years = _days / 365;
		const int months = (_days % 365) / 30;
		s.append(NX::itos<wchar_t>(years));
		s.append((years > 1) ? L" Years" : L" Year");
		if (0 != months) {
			s.append(L" ");
			s.append(NX::itos<wchar_t>(months));
			s.append((months > 1) ? L" Months" : L" Month");
		}
		s.append(L" Ago");
		return s;
	}
...
std::wstring time::timezone::serialize_bias() const
{
    ...
    swprintf_s(NX::wstring_buffer(s, 16), 16, L"%c%02d:%02d", (_bias > 0) ? L'-' : L'+', hours, minutes);
    return s;
}
...
// Time difference between 1601-01-01T00:00:00Z and 1970-01-01T00:00:00Z
#define TIME_DIFF   (116444736000000000LL)

time::datetime time::datetime::from_dos_time(__int64 dos_time)
{
    return  time::datetime((dos_time * 10000000) + TIME_DIFF);
}

__int64 time::datetime::to_dos_time() const
{
    __int64 dos_time = 0;
    if (_t > TIME_DIFF) {
        dos_time = (_t - TIME_DIFF) / 10000000;
    }
    return dos_time;
}

time::datetime time::datetime::from_unix_time(__int64 dos_time)
{
    return  time::datetime((dos_time * 10000) + TIME_DIFF);
}

__int64 time::datetime::to_java_time() const
{
    __int64 java_time = 0;
    if (_t > TIME_DIFF) {
        java_time = (_t - TIME_DIFF) / 10000;
    }
    return java_time;
}

time::datetime time::datetime::current_time()
{
    SYSTEMTIME st = { 0 };
    GetSystemTime(&st);
    return time::datetime(&st);
}

Result time::datetime::parse(const std::wstring& s)
{
    // ISO 8601
    //    [With Milliseconds]
    ...
    if (time::is_iso_8601_time_string(s)) {
        ...
        if (!time_str.empty()) {
            const wchar_t lz = time_str.c_str()[time_str.length() - 1];
            if (lz == L'Z' || lz == L'z') {
...
std::wstring time::datetime::serialize(bool local_time, bool with_milliseconds) const
{
    std::wstring s;
    SYSTEMTIME st = { 0 };
    ...
    if (local_time) {
        ...
        if (!with_milliseconds) {
            swprintf_s(NX::wstring_buffer(s, 64), 64, L"%04d-%02d-%02dT%02d:%02d:%02d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
        }
        else {
            swprintf_s(NX::wstring_buffer(s, 64), 64, L"%04d-%02d-%02dT%02d:%02d:%02d.%03d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
        }
        s += zone.serialize_bias();
    }
    else {
        if (!with_milliseconds) {
            swprintf_s(NX::wstring_buffer(s, 64), 64, L"%04d-%02d-%02dT%02d:%02d:%02dZ", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
        }
        else {
            swprintf_s(NX::wstring_buffer(s, 64), 64, L"%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
        }
    }
    return s;
}

Result time::datetime_rfc1123::parse(const std::wstring& s)
{
    // RFC 1123
    SYSTEMTIME st = { 0 };
...
std::wstring time::datetime_rfc1123::serialize(bool local_time, bool with_milliseconds) const
{
    std::wstring s;
    SYSTEMTIME st = { 0 };
    ...
    if (local_time) {
        ...
        swprintf_s(NX::wstring_buffer(s, 128), 128, L"%s, %02d %s %04d %02d:%02d:%02d %s",
            (st.wDayOfWeek < 0 || st.wDayOfWeek > 6) ? L"Ukn" : time_detail::utf16_weekday_names[st.wDayOfWeek],
            st.wDay,
            (st.wMonth < 1 || st.wMonth > 12) ? L"Ukn" : time_detail::utf16_month_names[st.wMonth - 1],
            st.wYear,
            st.wHour, st.wMinute, st.wSecond, (is_pm ? L"PM" : L"AM")
        );
    }
    else {
        swprintf_s(NX::wstring_buffer(s, 128), 128, L"%s, %02d %s %04d %02d:%02d:%02d GMT",
            (st.wDayOfWeek < 0 || st.wDayOfWeek > 6) ? L"Ukn" : time_detail::utf16_weekday_names[st.wDayOfWeek],
            st.wDay,
            (st.wMonth < 1 || st.wMonth > 12) ? L"Ukn" : time_detail::utf16_month_names[st.wMonth - 1],
            st.wYear,
...
Result time::datetime_special_1::parse(const std::wstring& s)
{
    // Special NextLabs Time
    //  "Feb 2, 2016 7:13:26 PM"
...
std::wstring time::datetime_special_1::serialize(bool local_time, bool with_milliseconds) const
{
    // Special NextLabs Time
    //  "Feb 2, 2016 7:13:26 PM"
    UNREFERENCED_PARAMETER(with_milliseconds);
    ...
    swprintf_s(NX::wstring_buffer(s, 128), 128, L"%s %d, %04d %02d:%02d:%02d %s",
        (st.wMonth < 1 || st.wMonth > 12) ? L"Ukn" : time_detail::utf16_month_names[st.wMonth - 1],
