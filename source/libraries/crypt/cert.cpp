...
Result CertContext::Create(const std::wstring& x500_name, unsigned long key_spec, bool strong, bool machineKeySet, _In_opt_ CertContext* issuer, const SYSTEMTIME& expire_date)
{
    HCRYPTPROV              hp = NULL;
    HCRYPTKEY               hk = NULL;
...
Result CertContext::Create(const unsigned char* pb, unsigned long cb)
{
...
Result CertContext::Create(const std::wstring& file)
{
    std::vector<UCHAR> certBlob;
    Result res = GetCertBinaryBlob(file, certBlob);
    if (!res) {
        return res;
    }
...
Result CertContext::CreateFromEmbeddedSignature(const std::wstring& host_file)
{
...
Result CertContext::Export(_In_ const std::wstring& file, _In_ bool base64_format)
{
    if (Empty()) {
...
Result CertContext::Duplicate(CertContext* pcc)
{
    if (Empty()) {
        return RESULT(ERROR_INVALID_HANDLE);
    }
...
Result CertContext::GetKeyBlob(std::vector<UCHAR>& blob)
{
    if (Empty()) {
...
Result CertContext::GetCertBlob(const std::wstring& file, std::vector<UCHAR>& blob)
{
...
Result CertContext::GetCertBinaryBlob(const std::wstring& file, std::vector<UCHAR>& blob)
{
    std::vector<unsigned char> buf;
    
    Result res = GetCertBlob(file, buf);
    if (!res) {
        return res;
    }
...
Result CertStore::EnumCertificates(ENUMCERTPROC EnumProc, PVOID context)
{
...
Result CertStore::AddCertificate(PCCERT_CONTEXT cc, DWORD addDisposition, CertContext* pcc)
{
    if (!Opened()) {
        return RESULT(ERROR_INVALID_HANDLE);
    }
...
Result CertSystemStore::OpenPersonalStore(bool machineStore)
{
    return Open(L"My", machineStore);
}

Result CertSystemStore::OpenTrustRootStore(bool machineStore)
{
    return Open(L"Root", machineStore);
}

Result CertSystemStore::OpenInterMediateStore(bool machineStore)
{
    return Open(L"CertificateAuthority", machineStore);
}

Result CertSystemStore::OpenTrustPublisherStore(bool machineStore)
{
    return Open(L"TrustedPublisher", machineStore);
}

Result CertSystemStore::OpenTrustPeopleStore(bool machineStore)
{
    return Open(L"TrustedPeople", machineStore);
}

Result CertSystemStore::OpenOtherPeopleStore(bool machineStore)
{
    return Open(L"AddressBook", machineStore);
}

Result CertSystemStore::OpenThirdPartyRootStore(bool machineStore)
{
    return Open(L"AuthRoot", machineStore);
}

Result CertSystemStore::OpenRevokedStore(bool machineStore)
{
    return Open(L"Disallowed", machineStore);
}

Result CertSystemStore::Open(const std::wstring& name, bool machineStore)
{
    Close();
...
Result CertPcks12FileStore::OpenPfxCert(const std::wstring& file, const std::wstring& password, bool exportable)
{
    std::vector<UCHAR> blob;

    Close();

    Result res = ReadCertContent(file, blob);
    if (!res) {
        return res;
    }
...
Result CertPcks12FileStore::ReadCertContent(const std::wstring& file, std::vector<UCHAR>& blob)
{
