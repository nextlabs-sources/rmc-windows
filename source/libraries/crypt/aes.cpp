...
Result AesKey::GenerateRandom(PUCHAR data, ULONG size)
{
...
Result AesKey::Generate(ULONG bitslength)
{
    Provider* prov = GetProvider(PROV_AES);
    if (NULL == prov) {
        return RESULT(ERROR_INVALID_HANDLE);
...
Result AesKey::Import(const UCHAR* key, ULONG size)
{
    Provider* prov = GetProvider(PROV_AES);
    if (NULL == prov) {
        return RESULT(ERROR_INVALID_HANDLE);
...
Result AesKey::Export(PUCHAR key, _Inout_ PULONG size)
{
    Provider* prov = GetProvider(PROV_AES);
    if (NULL == prov) {
        return RESULT(ERROR_INVALID_HANDLE);
...
Result NX::crypt::AesEncrypt(_In_ const AesKey& key,
...
Result NX::crypt::AesDecrypt(_In_ const AesKey& key,
    _In_reads_bytes_(in_size) const unsigned char* in_buf,
    _In_ unsigned long in_size,
    _Out_writes_bytes_opt_(*out_size) unsigned char* out_buf,
    _Inout_ unsigned long* out_size,
...
Result NX::crypt::AesBlockEncrypt(_In_ const AesKey& key,
    _In_reads_bytes_(in_size) const unsigned char* in_buf,
    _In_ unsigned long in_size,
    _Out_writes_bytes_opt_(*out_size) unsigned char* out_buf,
    _Inout_ unsigned long* out_size,
    ...
    _In_ const unsigned long cipher_block_size)
{
	...
	*out_size = 0;
    ...
    while (in_size) {
        ...
        Result res = AesEncrypt(key, in_buf, bytes_to_encrypt, out_buf, &output_size, ivec);
        if (!res)
            return res;
...
Result NX::crypt::AesBlockDecrypt(_In_ const AesKey& key,
    _In_reads_bytes_(in_size) const unsigned char* in_buf,
    _In_ unsigned long in_size,
    _Out_writes_bytes_opt_(*out_size) unsigned char* out_buf,
    _Inout_ unsigned long* out_size,
    ...
    _In_ const unsigned long cipher_block_size)
{
	...
	*out_size = 0;
    ...
    while (in_size) {
        ...
        UCHAR ivec[16] = { 0 };
        AesGenerateIvec(ivec_seed, currentOffset, ivec);
        Result res = AesDecrypt(key, in_buf, bytes_to_decrypt, out_buf, &output_size, ivec);
        if (!res)
            return res;
